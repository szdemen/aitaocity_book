...
$attachDir='upload_img';//上传文件保存路径，结尾不要带/
...
if($err==''){
	...
	if(preg_match('/'.str_replace(',','|',$upExt).'/i',$extension))
	{
		...
		else
		{
			...
			//else $msg="{'url':'".$targetPath."','localname':'".jsonString($localName)."','id':'1'}";  //原句
			else $msg="{'url':'".'/'.$targetPath."','localname':'".jsonString($localName)."','id':'1'}";//id参数固定不变，仅供演示，实际项目中可以是数据库ID, 此句返回编辑器图片的查看地址
		}
	}
